...
async function gradeBlock(ub_id, gradeButton) {
    ...
    try {
        ...
        if (vercelResponse.status !== 404) {
            ...
            console.error('Vercel grading error:', vercelResponse.status, errorData);
...
async function clearBlock(ub_id) {
    ...
    try {
        const xanoResponse = await fetch(`https://xxye-mqg7-lvux.n7d.xano.io/api:DwPBcTo5/clear_ub?ub_id=${ub_id}`, {
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json' }
        });

        if (!xanoResponse.ok) {
            throw new Error(`Clear request failed with status ${xanoResponse.status}`);
        }

        try {
            await fetch(`${WORKFLOW_API_URL}/chat/${ub_id}/state`, {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' }
            });
        } catch (stateError) {
            console.warn('Could not reset workflow state:', stateError);
        }
