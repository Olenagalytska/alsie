window.WORKFLOW_API_URL = window.WORKFLOW_API_URL || 'https://workflow-e6k2mai8x-toropilja374-gmailcoms-projects.vercel.app';
...
async function gradeTestBlock(ub_id, gradeButton) {
    ...
    try {
        // 1. Try Vercel API first (self-hosted workflows)
        ...
        console.log('Vercel API returned non-ok, trying Xano fallback...');

        // 2. Fallback: Xano API (old workflows)
...
async function gradeLessonTests(lesson_id, gradeLessonButton) {
    const originalText = gradeLessonButton ? gradeLessonButton.textContent : 'Grade Module';

    try {
        if (gradeLessonButton) {
            gradeLessonButton.textContent = 'Grading...';
            gradeLessonButton.disabled = true;
        }
        alert('Grading process started. Feel free to leave the page in a few seconds. Grading will continue in the background.');

        // 1. Try Vercel API first (self-hosted workflows)
        const vercelResponse = await fetch(`${WORKFLOW_API_URL}/lesson/${lesson_id}/evaluate-tests`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            keepalive: true
        });
        ...
        if (vercelResponse.ok) {
            const result = await vercelResponse.json();
            console.log('Vercel lesson grading successful:', result);
        ...
        // 2. Fallback: Xano API (old workflows)
        const xanoResponse = await fetch(`https://xxye-mqg7-lvux.n7d.xano.io/api:DwPBcTo5/grade_lesson_tests?lesson_id=${lesson_id}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            keepalive: true
        });
        ...
        console.log('Xano lesson grading successful');
        if (document.visibilityState === 'visible') {
            await refreshTestProgress();
        }
        ...
        if (gradeLessonButton && document.visibilityState === 'visible') {
            gradeLessonButton.textContent = originalText;
            gradeLessonButton.disabled = false;
        }
    }
}