...
async function initializeAssistantForm(blockData, block_id, lesson_id) {
    ...
    async function initializeTemplateHandling(blockData, templates, block_id, lesson_id) {
        ...
        // Always show evaluation section
    ...
    function showTemplateForm(templates) {
        ...
        // Show/hide sections - BUT ALWAYS SHOW EVALUATION SECTION
        if (evaluationSection) evaluationSection.style.display = 'block';
    ...
    function removeParameterSet(setToRemove) {
        const index = specificationsSets.indexOf(setToRemove);
        if (index !== -1) {
            setToRemove.container.remove();
            specificationsSets.splice(index, 1);
            
            // Update remaining set indices
