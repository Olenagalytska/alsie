class StudentChat {
  constructor() {
    ...
    this.appState = {
      ...
      selectedFile: null,
      workflowApiUrl: 'https://workflow-28go20xvp-toropilja374-gmailcoms-projects.vercel.app',
      messageIndex: 0
    };
  ...
  async initChatKit(workflowId) {
    ...
    if (!allowMultipleChats) {
      try {
        const threadsResponse = await fetch(`${workflowApiUrl}/chatkit/threads?ub_id=${ubId}`);
        if (threadsResponse.ok) {
          const threadsData = await threadsResponse.json();
          if (threadsData.threads && threadsData.threads.length > 0) {
            existingThreadId = threadsData.threads[0].id;
            console.log('[ChatKit] Found existing thread:', existingThreadId);
          }
        }
      } catch (error) {
    ...
    if (chatWidget) {
      ...
      if (isSelfHosted) {
        let apiUrl = `${workflowApiUrl}/chatkit?ub_id=${ubId}&block_id=${blockId}&user_id=${userId}&session_id=${uniqueSessionId}`;
        
        if (existingThreadId) {
          apiUrl += `&thread_id=${existingThreadId}`;
          console.log('[ChatKit] Loading existing thread via URL:', existingThreadId);
        }
  ...
  async loadChatHistory() {
    try {
      ...
      if (this.currentChatContext && 
          (this.currentChatContext.ubId !== currentContext.ubId || 
           this.currentChatContext.threadId !== currentContext.threadId)) {
        console.log('Chat context changed, clearing history');
        this.currentChatContext = currentContext;
        this.elements.mainContainer.innerHTML = '';
      }
      ...
      try {
        ...
        if (workflowResponse.ok) {
          ...
          if (workflowState.ub_id === this.appState.ubId && 
              workflowState.answers && 
              workflowState.answers.length > 0) {
      ...
      if (!hasMessages && this.appState.ubData?.thread_id) {
        try {
          ...
          if (response.ok) {
            ...
            if (messages && 
                messages.length > 0 && 
                this.appState.ubData.thread_id === currentContext.threadId) {
      ...
      if (!hasMessages) {
        console.log('No chat history found for current chat');
      }
