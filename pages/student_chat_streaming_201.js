class StudentChat {
  constructor() {
    ...
    this.appState = {
      ...
      selectedFiles: [],
      workflowApiUrl: 'https://workflow-qgfq63sax-toropilja374-gmailcoms-projects.vercel.app',
      messageIndex: 0
    };
          ...
          if (workflowState.ub_id === this.appState.ubId && 
            ...
            workflowState.answers.forEach((answer, index) => {
              ...
              if (answer.interviewer_question) {
                this.createAssistantMessage(answer.interviewer_question, ts);
              }

              if (answer.tutor_response) {
                this.createAssistantMessage(answer.tutor_response, ts);
              } else if (answer.assignment && !shownAssignments.has(answer.assignment)) {
                this.createAssistantMessage(answer.assignment, ts);
                shownAssignments.add(answer.assignment);
              }

              if (answer.coach_response) {
                this.createAssistantMessage(answer.coach_response, ts);
              }

              if (answer.assistant_response) {
                this.createAssistantMessage(answer.assistant_response, ts);
              }
              ...
              if (answer.answer) {
                this.createUserMessage(answer.answer, ts, answer.user_files || []);
              }
            });
