class StudentChat {
  constructor() {
    ...
    this.appState = {
      ...
      selectedFile: null,
      workflowApiUrl: 'https://workflow-hw6y4ggsp-toropilja374-gmailcoms-projects.vercel.app'
    };
  }
  ...
  setupFileUploadListeners() {
    this.elements.attachFile?.addEventListener('click', () => this.openFileDialog());
    this.elements.removeFile?.addEventListener('click', () => this.clearSelectedFile());
    this.elements.uploadFile?.addEventListener('click', async () => await this.uploadFile());
  }
  ...
  async uploadFile() {
    if (!this.appState.selectedFile) return;
  ...
  async loadChatHistory() {
    try {
        ...
        if (airData && airData.length > 0) {
          ...
          airData.forEach(item => {
            if (item.user_content) {
              ...
              if (userText) this.createUserMessage(userText);
            }
            if (item.ai_content) {
              let aiContent = item.ai_content;
              try {
                aiContent = typeof item.ai_content === 'string' ? JSON.parse(item.ai_content) : item.ai_content;
              } catch (e) {}
              if (Array.isArray(aiContent)) {
                aiContent.forEach(ai => { if (ai.text) this.createAssistantMessage(ai.text); });
              } else if (aiContent.text) {
                this.createAssistantMessage(aiContent.text);
              }
            }
          });
          hasMessages = true;
        ...
        if (workflowResponse.ok) {
          ...
          if (workflowState.answers && workflowState.answers.length > 0) {
            ...
            workflowState.answers.forEach(answer => {
              if (answer.user_message) this.createUserMessage(answer.user_message);
              const aiResponse = answer.assistant_response || answer.coach_response || answer.tutor_response || answer.assignment;
              if (aiResponse) this.createAssistantMessage(aiResponse);
            });
            hasMessages = true;
      ...
      if (!hasMessages) console.log('No chat history found');
    } catch (error) {
  ...
  displayMessage(message) {
    ...
    if (message.role === 'user') this.createUserMessage(messageText);
    else if (message.role === 'assistant') this.createAssistantMessage(messageText);
  }
    ...
    try {
      if (typeof marked !== 'undefined') {
        ...
        if (typeof Prism !== 'undefined') {
          aiText.querySelectorAll('pre code').forEach(block => Prism.highlightElement(block));
        }
      } else {
  ...
  async handleStudentSubmit(event) {
    ...
    try {
      ...
      if (this.appState.currentStreamingMessage) {
        this.appState.currentStreamingMessage.textContent = 'Sorry, there was an error processing your request.';
      }
    }
  }
  ...
  async startWorkflowStreaming(userInput) {
    try {
      const response = await fetch(`${this.appState.workflowApiUrl}/chat/message`, {
        ...
        body: JSON.stringify({ ub_id: this.appState.ubId, content: userInput })
      });
  ...
  finalizeStreamingMessage() {
    if (this.appState.currentStreamingMessage && this.appState.currentStreamingRawText) {
      ...
      if (container) this.setupCodeBlocks(container);
      this.scrollToBottom();
    }
    this.setUILoadingState(false);
