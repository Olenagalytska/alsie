class TeacherChat {
  constructor() {
    ...
    this.appState = {
      ...
      workflowApiUrl: 'https://workflow-1j7lzyail-toropilja374-gmailcoms-projects.vercel.app',
      studentName: null,
      messageIndex: 0,
      selectedFiles: []
    };
  ...
  async loadChatHistory() {
    try {
      ...
      try {
          ...
          if (workflowState.answers && workflowState.answers.length > 0) {
            workflowState.answers.forEach((answer) => {
              if (answer.user_message) {
                this.createUserMessage(answer.user_message, answer.timestamp || null, answer.user_files || []);
              }
              if (answer.agent_response) {
                this.createAssistantMessage(answer.agent_response, answer.timestamp || null);
              }
            });
        ...
        console.log('Workflow state not available:', workflowError);
      }
      
      if (!hasMessages && this.appState.ubData.thread_id) {
        try {
          const threadResponse = await fetch(`${this.appState.workflowApiUrl}/threads/${this.appState.ubData.thread_id}/messages`);
          
          if (threadResponse.ok) {
            const messages = await threadResponse.json();
            
            if (messages && messages.length > 0 && messages[0].thread_id === this.appState.ubData.thread_id) {
