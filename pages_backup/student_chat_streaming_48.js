...
class StudentChat {
  constructor() {
    ...
    this.appState = {
      ...
      selectedFile: null,
      workflowApiUrl: 'https://workflow-gsf8z2brc-toropilja374-gmailcoms-projects.vercel.app'
    };
  }
  ...
  async initChatKit(workflowId) {
    ...
    const checkChatKit = setInterval(() => {
      ...
      if (chatWidget && typeof chatWidget.setOptions === 'function') {
    ...
    setTimeout(() => {
      clearInterval(checkChatKit);
      console.warn('ChatKit widget initialization timeout');
    }, 10000);
  }
        ...
        if (workflowResponse.ok) {
          ...
          if (workflowState.answers && workflowState.answers.length > 0) {
            ...
            workflowState.answers.forEach((answerObj, index) => {
              ...
              // === ПОРЯДОК: спочатку AI, потім User ===
              
              // 1. Відповідь AI (assignment/question) - показуємо ПЕРШОЮ
              ...
              // 2. Повідомлення користувача - показуємо ДРУГИМ
